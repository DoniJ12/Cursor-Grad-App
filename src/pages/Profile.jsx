...
function Profile({ user }) {
  ...
  useEffect(() => {
    ...
    const nonEmptyStacks = savedStacks.filter(stack => stack.images.length > 0);
    setImageStacks(nonEmptyStacks);
    if (nonEmptyStacks.length !== savedStacks.length) {
      localStorage.setItem('imageStacks', JSON.stringify(nonEmptyStacks));
    }
  }, []);
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="space-y-6">
          {imageStacks.length === 0 ? (
            <p className="text-center text-gray-600">No image stacks yet. Start by uploading some images!</p>
          ) : (
